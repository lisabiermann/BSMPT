...
const std::vector<double> example_point_VDM{/* MH1 = */ 104.53780709965832,
                                            /* MH2 = */ 125.09,
                                            /* v = */ 246.21965079413738,
                                            /* vs = */ 512.4840306675902,
                                            /* alpha = */ 1.4723279429814098,
                                            /* MX = */ 566.421195897138};

constexpr auto Model = BSMPT::ModelID::ModelIDs::VDM;
const Compare_VDM Expected;

TEST_CASE("Checking NLOVEV for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
      ModelID::FChoose(Model, SMConstants);
  modelPointer->initModel(example_point_VDM);
...
TEST_CASE("Checking EWPT for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking number of CT parameters for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
      ModelID::FChoose(ModelID::ModelIDs::VDM, SMConstants);
  modelPointer->initModel(example_point_VDM);
  auto result = ModelTests::CheckNumberOfCTParameters(*modelPointer);
  REQUIRE(result == ModelTests::TestResults::Pass);
}

TEST_CASE("Checking number of VEV labels for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE(
    "Checking number of labels for temperature dependend results for VDM",
    "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Checking number of triple Higgs couplings for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking Gauge Boson masses for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking fermion and quark masses masses for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking tree level minimum for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking tree level tadpoles for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking NLO masses matching tree level masses for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking VTreeSimplified for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking VCounterSimplified for VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking first derivative of the sum of CT and CW in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Checking second derivative of the sum of CT and CW in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Checking triple higgs NLO couplings in the VDM", "[vdm]")
{
...
TEST_CASE("Check number of calculated CT parameters in the VDM", "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Check symmetric properties of the scalar tensor Lij in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the scalar tensor Lijk in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the scalar tensor Lijkl in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the gauge tensor in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the Lepton tensor in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the mass Lepton tensor in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the mass quark tensor in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the quark tensor in the VDM",
          "[vdm]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
