...
void Class_Potential_CPintheDark::set_gen(const std::vector<double>& par) {
	...
	m11s = par[0];
	m22s = par[1]; // Class member is set accordingly to the input parameters
	mSs = par[2];
	ReA = par[3]; 
	ImA = par[4];
	L1 = par[5];
...
void Class_Potential_CPintheDark::set_CT_Pot_Par(const std::vector<double>& par){
	...
	Curvature_Higgs_CT_L4[8][8][8][8] = 3 * dL6;
}
...
std::vector<double> Class_Potential_CPintheDark::calc_CT() const {
	...
	parCT.push_back(HesseWeinberg(4, 4) / 2 - 0.3e1 / 0.2e1 * HesseWeinberg(5, 5)); //dm11s
	parCT.push_back(-HesseWeinberg(3, 3)); //dm22s
	parCT.push_back(-HesseWeinberg(8, 8)); //dmSs
	...
	parCT.push_back(0); //dL2
	parCT.push_back(0); //dL3
	parCT.push_back((2 * HesseWeinberg(3, 3) - HesseWeinberg(6, 6) - HesseWeinberg(7, 7)) * pow(v1, -2)); //dL4
	parCT.push_back((-HesseWeinberg(6, 6) + HesseWeinberg(7, 7)) * pow(v1, -2)); //dL5
	parCT.push_back(0); //dL6
	parCT.push_back(0); //dL7
	parCT.push_back(0); //dL8
...
void Class_Potential_CPintheDark::SetCurvatureArrays(){
   ...
   */
	...
	Curvature_Higgs_L4[8][8][8][8] = 3 * L6;
