...
#include <BSMPT/models/ClassPotentialRxSMEFT.h>
 ...
 */
Class_RxSMEFT::Class_RxSMEFT(const ISMConstants &smConstants)
    : Class_Potential_Origin(smConstants)
{
  Model =
      ModelID::ModelIDs::RXSMEFT; // global int constant which will be used to
 ...
 */
std::vector<std::string> Class_RxSMEFT::addLegendCT() const
{
 ...
 * input file
 */
std::vector<std::string> Class_RxSMEFT::addLegendTemp() const
{
  std::vector<std::string> labels;
 ...
 *
 */
std::vector<std::string> Class_RxSMEFT::addLegendTripleCouplings() const
{
  std::vector<std::string> labels;
 ...
 */
std::vector<std::string> Class_RxSMEFT::addLegendVEV() const
{
  std::vector<std::string> labels;
  // out = "Your VEV order";
 ...
 */
void Class_RxSMEFT::ReadAndSet(const std::string &linestr,
...
void Class_RxSMEFT::set_gen(const std::vector<double> &par)
{
  ...
  lambda = 2 * mh2 / vH / vH +
           6 * vS * vS * lambdaHS * lambdaHS / (vS * vS * lambdaS - 3 * mh2);
 ...
 */
void Class_RxSMEFT::set_CT_Pot_Par(const std::vector<double> &par)
{
 ...
 */
void Class_RxSMEFT::write() const
{
 ...
 */
std::vector<double> Class_RxSMEFT::calc_CT() const
{
  ...
  if (UnbrokenSingletPhase)
  {
    ...
    // dlambda
    parCT.push_back((2 * HesseWeinberg(2, 2) - 2 * HesseWeinberg(3, 3)) / vH /
                    vH);
  ...
  else
  {
    ...
    // dmsq
    parCT.push_back((-3 * HesseWeinberg(2, 2) * vH + HesseWeinberg(3, 3) * vH +
                     HesseWeinberg(3, 4) * vS) /
                    vH);
    ...
    parCT.push_back((HesseWeinberg(3, 4) * vH + HesseWeinberg(4, 4) * vS -
                     3 * NablaWeinberg(4)) /
                    vS / 2);

    // dlambdaS
    // parCT.push_back(0);
    parCT.push_back((-3 * HesseWeinberg(4, 4) * vS + 3 * NablaWeinberg(4)) *
                    pow(vS, -3));
 ...
 */
void Class_RxSMEFT::AdjustRotationMatrix()
{
  const double ZeroThreshold = 1e-5;
  ...
  int pos_rho1 = 0, pos_eta1 = 1, pos_psi1 = 2, pos_zeta1 = 3, pos_zetaS = 4;
  ...
  if (pos_G1 == -1 or pos_G2 == -1 or pos_G0 == -1 or pos_h == -1 or
      pos_H == -1)
  {
...
void Class_RxSMEFT::TripleHiggsCouplings()
{
  if (!SetCurvatureDone) SetCurvatureArrays();
  if (!CalcCouplingsDone) CalculatePhysicalCouplings();
...
void Class_RxSMEFT::SetCurvatureArrays()
{
  initVectors();
...
bool Class_RxSMEFT::CalculateDebyeSimplified()
{
  return false;
  /*
...
bool Class_RxSMEFT::CalculateDebyeGaugeSimplified()
{
  /*
...
double Class_RxSMEFT::VTreeSimplified(const std::vector<double> &v) const
{
  (void)v;
...
double Class_RxSMEFT::VCounterSimplified(const std::vector<double> &v) const
{
  (void)v;
...
void Class_RxSMEFT::PerformVCTShift()
{
  // not implemented
}

void Class_RxSMEFT::Debugging(const std::vector<double> &input,
