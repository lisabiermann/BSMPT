// Copyright (C) 2024  Philipp Basler, Lisa Biermann, Margarete Mühlleitner and
// Jonas
// SPDX-FileCopyrightText: 2021 Philipp Basler, Margarete Mühlleitner and
// Jonas Müller
//
...
/**
 * @file
 * Calculates the Quartic Higgs couplings for mulitple points and adds them at
 * the end of the line.
 */
  ...
  while (getline(infile, linestr))
  {
    ...
    if (linecounter == 1)
    {
      ...
      for (auto x : modelPointer->addLegendCT())
        outfile << sep << x;
      for (auto x : modelPointer->addLegendQuarticCouplings())
        outfile << sep << x;
      outfile << std::endl;
    }
        ...
        linecounter != 1)
    {
      ...
      modelPointer->Prepare_Quartic();
      modelPointer->QuarticHiggsCouplings();
      ...
      for (std::size_t i = 0; i < NHiggs; i++)
      {
        for (std::size_t j = i; j < NHiggs; j++)
        {
          for (std::size_t k = j; k < NHiggs; k++)
          {
            ...
            {
              outfile << sep
                      << -modelPointer->get_QuarticHiggsCorrectionsTreePhysical(
                             i, j, k, l);
              outfile << sep
                      << -modelPointer->get_QuarticHiggsCorrectionsCTPhysical(
                             i, j, k, l);
              outfile << sep
                      << -modelPointer->get_QuarticHiggsCorrectionsCWPhysical(
                             i, j, k, l);
            }
          }
        }
...
BSMPT::parser prepare_parser()
{
  ...
  std::stringstream ss;
  ss << "QuarticHiggsNLO calculates the coupling between three Higgs bosons"
     << std::endl
     << "It is called either by " << std::endl
     << "./QuarticHiggsNLO Model Inputfile Outputfile LineStart LineEnd"
     << std::endl
