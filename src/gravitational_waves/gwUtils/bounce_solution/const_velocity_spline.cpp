...
#include <BSMPT/gravitational_waves/gwUtils/bounce_solution/const_velocity_spline.h>
...
void cvspline::add_point(double p, std::vector<double> new_vev, bool compile)
{
         ...
         1) // When upper - lower = 1 then we add the point at that position
  {
        ...
        (vev_position[lower] == lower_temp))
    {
      ...
      std::cout << "Point in that position already exists!\n";
      return;
    }
...
void cvspline::print_path()
{
  ...
  std::cout << std::string(15 * (dim + 2), '-') << std::endl;
  std::cout << std::setw(5) << std::right << "id" << std::setw(wid)
            << std::right << "l";

  for (int i = 0; i < dim; i++)
  {
    std::cout << std::setw(wid) << std::right << ("Field " + std::to_string(i))
  ...
  for (int i = 0; i < num_points; i++)
  {
    ...
    std::cout << std::setw(5) << std::right << i << std::setw(wid) << std::right
              << vev_position[i] << std::setw(wid) << std::right;
    for (int j = 0; j < dim; j++)
    {
      std::cout << phipath[i][j] << std::setw(wid) << std::right;
    }
    std::cout << " " << std::endl;
  }
  std::cout << std::string(15 * (dim + 2), '-') << std::endl;
