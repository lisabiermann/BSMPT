// Copyright (C) 2022 Lisa Biermann
...
{
// Simple library to ease vectorial calculations
template <typename T>
                                                 ...
                                                 std::vector<double> gradient)
{
  ...
  r = std::pow(dldrho, 2) * d2phidl2 - (gradient - (gradient * dphidl) * dphidl);
...
double BounceActionInt::Bernstein(int n, int nu, double x)
{
  ...
  return (nChoosek(n, nu) * std::pow(x, nu) * std::pow(1 - x, n - nu));
  BSMPT::Logger::Write(BSMPT::LoggingLevel::GWDetailed, ss.str());
}
...
void BounceActionInt::SinglePathDeformationCosmo(
    double &stepsize,
    double &reductor,
    std::vector<double> &l,
    tk::spline &rho_l_spl,
    ...
    std::vector<std::vector<double>> &forces)
{
  ...
  ss << "Path deformation error (before "
        "integrating)\t"
     << MaximumRelativeError;
  BSMPT::Logger::Write(BSMPT::LoggingLevel::GWDetailed, ss.str());
  ss.str(std::string());
  ...
  {
    best_path = next_path;
    ss << "Next best path found!\t" << MaximumRelativeError;
    BSMPT::Logger::Write(BSMPT::LoggingLevel::GWDetailed, ss.str());
    ss.str(std::string());
  }
  else
...
void BounceActionInt::PathDeformationCosmo(std::vector<double> &l,
                                           tk::spline &rho_l_spl)
{
  std::stringstream ss;
  // First try at path deformation a
  ...
  int AbsoluteCounter =
      0; // Absolute counter. If the error decreases we add one more atempt.
  // This makes sure it does not get stuck in a loop.
  ...
  for (int it_maxpath = 0; it_maxpath < MaxSinglePathDeformations; it_maxpath++)
  {
    ...
    SinglePathDeformationCosmo(stepsize,
                               reductor,
                               l,
                               rho_l_spl,
                               l_fornextpath,
    ...
    {
      ss << "Enough convergence!\t" << MaximumRelativeError;
      BSMPT::Logger::Write(BSMPT::LoggingLevel::GWDetailed, ss.str());
      ss.str(std::string());
      break;
    }
    ...
    /*if (it_maxpath == MaxSinglePathDeformations - 1 and
        AbsoluteCounter < MaxSinglePathDeformations + 50)
    {
      // One more try
      ss << "Algorithm still converging. Try one more.\n";
      it_maxpath--;
    }*/
  }
  ...
  {
    ss << "Maximum iterations reached without error increasing. Integrate "
          "again!\n";
  }
    ...
    double error) // alpha = 2 at T > 0 and alpha = 3 at T = 0
{
               ...
               // bounce equation
  {
    ...
    BSMPT::Logger::Write(BSMPT::LoggingLevel::GWDetailed,
                         "--------------------------------\t1\t----------------"
                         "-----------------");
    Solve1DBounce(rho, l, dl_drho, d2l_drho2,
    ...
    for (int i = 2; i <= MaxPathIntegrations and convPathDeformation == -1; i++)
    {
      ...
      // PathDeformationQuarticGradientFlow(l, l_rho_spl, rho_l_spl);
      PathDeformationCosmo(l, rho_l_spl);
  ...
  for (double r = 0.0; r <= rho[rho.size() - 1]; r += int_delta)
  {
    ...
    integral +=
        r * r * (0.5 * std::pow(dl_drho_spl(r), 2) + V(spline(l_rho_spl(r))));
    integral += 4 * r * r *
                (0.5 * std::pow(dl_drho_spl(r + int_delta / 2.0), 2) +
                 V(spline(l_rho_spl(r + int_delta / 2.0))));
    integral += r * r *
                (0.5 * std::pow(dl_drho_spl(r + int_delta), 2) +
                 V(spline(l_rho_spl(r + int_delta))));
  }
  action = integral * 4 * M_PI * int_delta /
